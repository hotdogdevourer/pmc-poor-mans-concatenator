...
#define VBC_PUSH(raw_unsigned, max_val) do { \
    ...
    _s = (int32_t)(((uint64_t)(raw_unsigned) * 65535ULL / (max_val))) - 32768; \
                           ...
                           int verbose, int werror, int iwarn)
{
    ...
    if (verbose) {
        ...
        printf("[INFO] Loaded VBC: %u entries, %uHz, %u-bit %s, %zu bytes\n",
               entry_count, vb->framerate, vb->bit_depth, fmt_str, vb->data_size);
    }
    return 1;
...
static int tokenize_text(const char *text, const VoiceBank *vb, TokenList *out) {
    ...
    char *lower = (char *)malloc(tlen + 1u);
    if (!lower) return 0;
    for (size_t i = 0; i <= tlen; i++)
        lower[i] = (text[i] >= 'A' && text[i] <= 'Z') ? text[i] + 32 : text[i];

    uint32_t ec = vb->entry_count;
    char    **stripped = (char **)malloc(ec * sizeof(char *));
    const char **original = (const char **)malloc(ec * sizeof(const char *));
    ...
    uint32_t *order = (uint32_t *)malloc(ec * sizeof(uint32_t));
    if (!order) { free(lower); return 0; }
    for (uint32_t k = 0; k < ec; k++) order[k] = k;

    for (uint32_t a = 1; a < ec; a++) {
    ...
    for (uint32_t k = 0; k < ec; k++) free(stripped[k]);
                               ...
                               int verbose, int werror, int iwarn)
{
    ...
    { double ratio = total_original > 0u ? (double)data_used / (double)total_original : 0.0;
      printf("Created %s (%zu bytes %d-bit audio, %zu mappings)\n",
             output_file, data_used, bit_depth, valid_count);
      if (total_original > 0u)
          printf("Original: %zu bytes, Compression: %.1f%% of original\n",
                 total_original, ratio * 100.0);
      printf("CRC32: %08X\n", crc);
    }
    if (verbose) printf("[INFO] %zu/%zu entries packed successfully\n", valid_count, n_entries);
                  ...
                  int verbose, int werror, int iwarn)
{
    ...
    if (skipped > 0u && !iwarn) printf("[INFO] Skipped %zu unmapped token(s)\n", skipped);
    if (valid == 0u) { fprintf(stderr, "[ERROR] No valid segments after filtering.\n"); return 0; }
    if (verbose) printf("Stitching %zu segment(s)...\n", valid);
    ...
    if (gen_mode) {
        ...
        { MapList ml; PackEntry *entries; char tmp_dir[] = "_vbc_gen_tmp"; int used_tmp = 0; int ok;
          ...
          { const char *map_path = sg_map ? sg_map : cvb_map;
            ...
            printf("Loaded %zu map entries from %s\n", ml.count, map_path);
          }
